...
public class Question4 {
    ...
    public static int[] solution(int s, int[] arr) {
        int[] cMemory = new int[s];

        // 배열 안에 숫자 있다 ? 인덱스 받아서 다시 앞으로 추가 > 나머지 숫자 밀기
        // 배열 안에 숫자 없다 ? 나머지 숫자 밀기 > 맨 앞에 숫자 추가
        for (int i = 0; i < arr.length; i++) {
            int idx = findIndex(arr[i], cMemory);
            if (idx != -1) {
                // 배열에 숫자 있는 경우
                for (int y = idx-1; y >= 0; y--) {
                    cMemory[y+1] = cMemory[y];
                }
                cMemory[0] = arr[i];
            } else {
                // 배열에 숫자 없는 경우
